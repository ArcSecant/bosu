...
sliderRate curTime sm tps' =
    let tps = reverse $ filter (\x -> beatLength x <= 0) tps'
     in sliderRate' tps
  where
    sliderRate' ts
        | null ts = 270 * sm
        ...
        getSliderPaths (h : hs) acc = case details h of
            Slider shape info pLength -> case shape of
                ...
                Bezier points ->
                  ...
                  where
                    newHead = toFloatPair (position h) : toVec (head points)
                Perfect a b ->
                    getSliderPaths hs $
...
getVisibleObjs curTime game = filter isVisible (allObjs game)
  where
    ...
    isVisible ho = case details ho of
        ...
        Slider _ info pLength -> toSeconds ho - 0.75 <= curTime && toSeconds ho + addTime >= curTime
          where
            addTime = fromIntegral (repeats info) * realToFrac pLength / sliderRate (toSeconds ho) sm tps

renderHitObjects :: BOsu -> [Picture]
renderHitObjects game =
    let (sm, tps, hitObjs) = (curSM game, timing game, curObjsToHit game)
     in map renderHObj hitObjs
  where
    renderHObj ho = case details ho of
        HitCircle -> hCircle (position ho) 30
        Spinner endTime -> hCircle (256, 192) (300 * r)
          where
            r = min 1 $ (fromIntegral endTime / 1000 - elapsedTime game) / (fromIntegral endTime / 1000 - toSeconds ho)
        Slider shape _ pLength -> case shape of
            ...
            Bezier points -> drawBezier (realToFrac pLength) $ map (map reCenter) $ newHead : map toVec (tail points)
              where
                newHead = toFloatPair (position ho) : toVec (head points)
...
makeApproachCirc game = map makeApproachCirc' (curObjsToHit game)
  where
    ...
    makeApproachCirc' ho =
        let r = max 1 (1.0 + 2.0 * (toSeconds ho - curTime) / 0.75)
         in case details ho of
                Spinner _ -> Blank
                _ -> if r == 1 then Blank else aCircle (position ho) (30.0 * r)

renderSliderball :: BOsu -> [Picture]
renderSliderball game = map renderSlider (curObjsToHit game)
  where
    ...
    renderSlider ho = case details ho of
        Slider shape info pLength -> case shape of
          ...
          where
            renderSliderball' ho path' =
                let (curX, curY) = getSliderballPos game path' (Slider shape info pLength) (toSeconds ho)
                    sliderBall = Translate (curX -256) (curY -192) $ Color white $ ThickCircle 25 7.5
                 in
                     if curTime > toSeconds ho && renderSliderFollow game
                         then Pictures [sliderBall, Translate (curX - 256) (curY - 192) $ Color (greyN 0.75) $ ThickCircle 60 2.5]
                         else sliderBall
        _ -> Blank

getSliderballPos :: BOsu -> Seq.Seq Vector -> HitObjectDetails -> Float -> Vector
getSliderballPos game path' (Slider shape info pLength) objStartTime =
    let (curTime, sm, tps) = (elapsedTime game, curSM game, timing game)
        path'' n =
            if even n
                then (path' >< Seq.reverse path') >>= Seq.replicate (n `div` 2)
                else path'' (n - 1) >< path'
...
renderHitFeedback :: BOsu -> Picture
renderHitFeedback game = case remainingObjs game of
    [] -> Blank
    o : _ ->
        let key = getKey o
            (t, (x, y)) = key
         ...
         in case details o of
                HitCircle ->
                    if curTime >= hitTime && hitTime + 0.1 > curTime
                        then Translate (x - 256) (y - 192) $ renderHit scoreVal
                        else Blank
                Slider _ info pLength ->
                    if curTime >= hitTime + addTime && hitTime + addTime + 0.1 > curTime
                        then case Map.lookup key (sliderPaths game) of
                            Nothing -> Blank
                            Just sliderPath -> let _ :|> (x, y) = sliderPath in Translate (x - 256) (y - 192) $ renderHit scoreVal
                        else Blank
                  where
                    addTime = fromIntegral (repeats info) * realToFrac pLength / sliderRate (toSeconds o) sm tps
                Spinner endTime -> Blank
  where
    (curTime, hScores) = (elapsedTime game, hitScores game)
    (sm, tps) = (curSM game, timing game)

checkHit :: KeyState -> Vector -> BOsu -> BOsu
checkHit keyState (x, y) game =
    if null $ remainingObjs game
        then game{renderSliderFollow = False}
        else
            let curObj = head (remainingObjs game)
                key = getKey curObj
             in case details curObj of
                    HitCircle ->
                        if Map.member key (hitScores game)
                            then game{renderSliderFollow = False}
                            else
                                game
                                    { hitScores = Map.insert key (getHitVal curObj, elapsedTime game) (hitScores game)
                                    , renderSliderFollow = False
                                    }
                    Spinner endTime -> game{renderSliderFollow = False}
                    sliderInfo -> case Map.lookup (getKey curObj) (sliderPaths game) of
                        Nothing -> game{renderSliderFollow = False}
                        Just sliderPath ->
                            let pos = getSliderballPos game sliderPath sliderInfo (toSeconds curObj)
                             in if keyState == Down && distSqr pos (x + 256, y + 192) <= 100 ** 2
                                    then
                                        game
                                            { hitScores = Map.insert (toSeconds curObj, toFloatPair $ position curObj) (S100, elapsedTime game) (hitScores game)
                                            , renderSliderFollow = True
                                            }
                                    else game{renderSliderFollow = False}
  where
    -- Baby's first arrow
...
handleInput (EventKey (Char c) state _ pos) game =
    if c == 'z' || c == 'x'
        then checkHit state pos game
        else game
...
update tDelta game =
    let curTime = elapsedTime game + tDelta
        ...
        newRemObjs =
            let remObjs = remainingObjs game
             in case remObjs of
                    [] -> []
